...
//function route_process_block()
function route_process_block(blockhash, i) {
    if (!blockhash)
    	return;
	if (blockhash){
		      ...
		      if (blockhash == settings.genesis_block) {
		        ...
		        db.get_txs(block, function(txs) {
		          if (txs.length > 0) {
		        	...
		        	if (i == settings.numbersync)
		        		return;
		        	else{
	        		  db.create_txs(block, function(){
			               i++;
			               if (block.nextblockhash){
			            	   route_process_block(block.nextblockhash, i);
			               }
...
//function route_syncblock
//get lastblock on db => get blockhash lastest
function route_syncblock(res) {
	var arr = db.get_bockhash_latest(function(txs) {
		if (txs.length >0){
			getblock = lib.get_block(txs[0].blockhash, function (currentblock) {
				route_process_block(currentblock.nextblockhash, 0);
			});
		}
	});
...
/* GET for cronjob auto sync block from node wallet . */
router.get('/syncblock', function(req, res) {
  route_syncblock(res, null);
});
